...
public class Crawler implements Runnable {
    ...
    private static int maxPages = 15;     //max pages to stop at while crawling
    ...
    public Crawler(Database DB, Set<String> Visited, List<String> queue, Map<String, Vector<String>> forbiddenList, Map<String, Vector<String>> allowedList, int id) throws IOException {
        ...
        this.id = id;
        this.DB = DB;
        this.fileWriter = new FileWriter("src/main/java/Indexer/sites.txt", true); // "true" appends to the file
    ...
    public void crawl() {
        ...
        while (true) {
            ...
            if (currentUrl != null) {   //if not visited before, start scraping the url
                ...
                writeToFile(scraper.getLinks(), this.bufferedWriter, this.lock);
            }
        }
        Date time = new Date();
        DB.updateDate(time);
    ...
    public void writeToFile(List<String> strings, BufferedWriter writer, Object lock) {
        synchronized (lock) {
            try {
